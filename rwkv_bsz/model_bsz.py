...
class RWKV(MyModule):
    ...
    @MyFunction
    def ffn_one(self, x, sx, ln_w, ln_b, k_mix, r_mix, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry,lengs):
        #B = x.shape[0]
    ...
    @MyFunction
    def ffn_seq(self, x, sx, ln_w, ln_b, k_mix, r_mix, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, lengs):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        sx = torch.cat((sx.unsqueeze(1), xx[:, :-1, :]), 1)
        kx = xx * k_mix + sx * (1 - k_mix)
        rx = xx * r_mix + sx * (1 - r_mix)
        ...
        return x + out, xx[torch.arange(x.size(0)), lengs, :]
    ...
    @MyFunction
    def att_one(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory, lengs):
    ...
    @MyFunction
    def att_seq(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory, lengs):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        sx = torch.cat((sx.unsqueeze(1), xx[:, :-1, :]), 1)
        #
        kx = xx * k_mix + sx * (1 - k_mix)
        ...
        for t in range(T):
            ...
            e1 = torch.exp(pp - p)
            e2 = torch.exp(ww - p)
            sx[:, t, :] = (((e1 * aa + e2 * vv) / (e1 * bb + e2)).to(dtype=x.dtype))*(lengs >= t).unsqueeze(1).int() + sx[:, t, :]*(lengs < t).unsqueeze(1).int()
            ww = t_decay + pp
            p = torch.maximum(ww, kk)
            e1 = torch.exp(ww - p)
            e2 = torch.exp(kk - p)
            aa = (e1 * aa + e2 * vv)*(lengs >= t).int().unsqueeze(1) + aa*(lengs < t).int().unsqueeze(1)
            bb = (e1 * bb + e2)*(lengs >= t).int().unsqueeze(1) + bb*(lengs < t).int().unsqueeze(1)
            pp = p*(lengs >= t).int().unsqueeze(1) + pp*(lengs < t).int().unsqueeze(1)
        out = (r * sx) @ ow
        return x + out, xx[torch.arange(x.size(0)), lengs, :], aa, bb, pp
    ...
    def forward(self, tokens, state,lengs, full_output=False):
        with torch.no_grad():
            lengs = lengs-1
            w = self.w
            args = self.args
            ...
            if state == None:
                ...
                for i in range(args.n_layer): # state: 0=att_xx 1=att_aa 2=att_bb 3=att_pp 4=ffn_xx
                    ...
                    dev = dd.device
                    atype = dd.atype
                    state[i*5+0] = torch.zeros((B, args.n_embd), dtype=atype, requires_grad=False, device=dev).contiguous()
                    state[i*5+1] = torch.zeros((B, args.n_embd), dtype=torch.float, requires_grad=False, device=dev).contiguous()
                    state[i*5+2] = torch.zeros((B, args.n_embd), dtype=torch.float, requires_grad=False, device=dev).contiguous()
                    state[i*5+3] = torch.zeros((B, args.n_embd), dtype=torch.float, requires_grad=False, device=dev).contiguous() - 1e30
                    state[i*5+4] = torch.zeros((B, args.n_embd), dtype=atype, requires_grad=False, device=dev).contiguous()
            seq_mode = len(tokens[0]) > 1
